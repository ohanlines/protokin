...
## Deployment Model
Use one cloud deployment for production only.

Local testing:
- local DB + local config (`configs/config.dev.edn`)
- no deploy on push to `main`

Production deploy:
- release-gated by git tag only (example: `v0.1.0`)
- set `ADMIN_TOKEN` in production env for internal endpoint auth
- CI/CD workflow: `.github/workflows/release-deploy.yml`

Config templates:
- `configs/config.dev.edn.example`
- `configs/config.prod.edn.example`

Create real files from templates:
- `configs/config.dev.edn`
- `configs/config.prod.edn`

Run commands:
- dev migrate: `./scripts/migrate-dev-local.sh`
- dev run: `./scripts/run-dev-local.sh`
- prod migrate: `./scripts/migrate-prod.sh`
- prod run: `./scripts/run-prod.sh`
- prod release deploy: `./scripts/deploy-prod-release.sh vX.Y.Z`
- smoke check: `./scripts/smoke-check.sh http://localhost:8080`
